 ...
 * 
 */
public class QuotationViewOptionComposite extends Composite implements
		SelectionListener, IQuotationConstants, IUIConstants {
	 ...
	 * @param style
	 */
	public QuotationViewOptionComposite(Composite parent, int style) {
		super(parent, style);
		try {
	...
	public Composite loadComposite() {		
		...
		try {
			...
			rdQuick.setFont(SWTResourceManager.getFont("Tahoma", 9, 1, false,
					false));		
			composite = new QuickEditComposite(this, SWT.BORDER, quotList).loadComposite();			
	...
	private void handlePreviousAction() {
		...
		//Load composite		
		composite = new ManualEditComposite(this, SWT.BORDER, index, quotationDTO, quotList).loadComposite();
		composite.setBounds(15, 35, 800, 500);
		...
		try {
			...
			if ("Finish".equals(btnNext.getText())) {
				...
				if (validateComposite()) {
					...
					cptManual = (ManualEditComposite) composite;
					if(cptManual != null){
	...
	public void widgetSelected(SelectionEvent event) {
		...
		if (rdQuick == source) {
			...
			composite = (ManualEditComposite) new ManualEditComposite(this, SWT.BORDER, 0, quotationDTO, quotList).loadComposite();			
			composite.setBounds(15, 35, 800, 500);			
	...
	public void updateQuotation() {
		try {
			...
			QuotationHandler handler = new QuotationHandler();
