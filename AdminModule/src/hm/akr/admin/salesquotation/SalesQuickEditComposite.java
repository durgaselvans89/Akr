...
public class SalesQuickEditComposite extends Composite implements SelectionListener, IUIConstants {
	 ...
	 * @param style
	 */
	public SalesQuickEditComposite(Composite parent, int style, QuotationDTO[] quotList) {
		super(parent, style);
		try {
		...
		if (source == btnSet) {			
			...
			try {
				if(hMap.size() == 0){
					...
					SalesQuotationViewOptionComposite.isBpiChanged = true;
