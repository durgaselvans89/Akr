 ...
 * @version 1.0
 **/
public class FloatExtendedValidation implements VerifyListener {
	...
	public void verifyText(VerifyEvent event) {
		...
		if(temp.length() > 1){		
			...
			for (int i = 0; i < len; i++) {
				...
				if(dotindex > 0){
					if(!Character.isDigit(myChar) && myChar != '\b' && myChar != '.' && myChar != 'n' && myChar != 'N' && myChar != 'C'&& myChar != 'V' || 
							myChar != '\b' && ((decimalLen > 2) && cursorPosition > dotindex)){
						event.doit = false;
						return;
					}
				}else if (!Character.isDigit(myChar) && myChar != '\b' && myChar != '.' ) {
					event.doit = false;
					return;
				}
			}		
