...
public class LRCompositeHandler {
	...
	/*public BookingDTO[] getUsedLR(Date allocationDate, String type) throws Exception {
		...
		try {
			if (null != beanutil) {
				...
				usedlrDTO = remote
						.getUsedLRNumbers(stationCode, allocationDate, type);
			}
...
public BookingDTO[] getUnUsedLR() throws Exception{		
		remote = beanutil.getGoodsBean();
		return remote.getUnUsedLRList(getStationCode());		
	}

public BookingDTO[] getOldlrstatus(String lrno, String stationcode) throws Exception{
	// TODO Auto-generated method stub
	BookingDTO[] oldlrstatus = null;
	remote =beanutil.getGoodsBean();
	try{
		oldlrstatus = remote.getOldlrstatus(lrno,stationcode);
		System.out.println("getoldlrstatus----->"+oldlrstatus);
	}
	catch (Exception e) {
	...
	public Float getOldLrTotal(String lr_no){
		// TODO Auto-generated method stub
		
		Float  oldLrTotal = 0.0f;
		try{
			report = beanutil.getReportingBean();
			oldLrTotal= report.getOldLrTotal(lr_no);
		}
		catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
	...
	public GMRDTO getoldLrGMROdetails(String lrno, String from) {
		// TODO Auto-generated method stub
		GMRDTO gmrlist = null;
		try{
			remote = beanutil.getGoodsBean();
			gmrlist = remote.getoldLrGMROdetails(lrno,from);
		}catch (Exception e) {
			e.printStackTrace();
			// TODO: handle exception
		}
	...
	public boolean deliver_oldlr_goods(GMRDTO gLIST) throws Exception {
		boolean status = true;
		try {
			if (null != beanutil) {
				Goods remote = beanutil.getGoodsBean();
				remote.deliver_oldlr_goods(gLIST);
			}
		} catch (RemoteException remoteexception) {
