...
import hm.akr.container.cr.CashRegisterContainerHandler;
import hm.akr.container.cr.CashRegisterUI;
 ...
 * @version 1.0
 */
public class CashRegisterCompositePop extends Composite {

	CashRegisterContainerHandler handler = null;

	LRCompositeHandler lrhandler = null;

	String[] crTabType = new String[] { "CR UPDATE", "CR CANCEL" };
	 ...
	 * @param shell
	 */
	public CashRegisterCompositePop(Shell shell, int swtValue, String type) {
	...
	public Composite loadComposite() {
		try {
			if (shell != null) {
				...
				shell.setBounds(12, 130, 1000, 600);
				shell.open();
				shell.setText("CASH RECEIPT");
			...
			{
				crTab = new TabFolder(composite, SWT.NONE);
				crTab.setBounds(10,40,970,470);
				...
				populateCashDetails(type, false);
				...
				crTab.addSelectionListener(new SelectionListener() {
					...
					public void widgetSelected(SelectionEvent arg0) {
						...
						if (crTabType[0].equals(selectedTab)) {
	...
	private Canvas getCRCreateCanvas() {
		...
		if(type.equalsIgnoreCase("topay")){
			...
			crcpaid.setSelection(false);
			crcbilling.setSelection(false);
		}else if(type.equalsIgnoreCase("paid")){
			crcpaid.setSelection(true);
			crctopay.setSelection(false);
			crcbilling.setSelection(false);
		}else if(type.equalsIgnoreCase("billing")){
			crcbilling.setSelection(true);
			crcpaid.setSelection(false);
			crctopay.setSelection(false);
		}
